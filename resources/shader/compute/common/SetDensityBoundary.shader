...
void main() {
    ...
    // (1, 0) ~ (N, 0)
    density[accurate_workgroup_xy.x * ACTUAL_GRID_WIDTH] = density[accurate_workgroup_xy.x * ACTUAL_GRID_WIDTH + 1];
    // (1, N+1) ~ (N, N+1)
    density[accurate_workgroup_xy.x * ACTUAL_GRID_WIDTH + (VIRTUAL_GRID_WIDTH + 1)] = density[accurate_workgroup_xy.x * ACTUAL_GRID_WIDTH + VIRTUAL_GRID_WIDTH];
    // (0, 1) ~ (0, N)
    ...
    // (0, N+1)
    density[(VIRTUAL_GRID_WIDTH+1)] = 0.5 * (density[(VIRTUAL_GRID_WIDTH+1) + ACTUAL_GRID_WIDTH] + density[VIRTUAL_GRID_WIDTH]);
    // (N+1, N+1)
