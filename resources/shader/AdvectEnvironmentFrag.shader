...
void main() {
    // Scalar
    // TODO accurate_xy
    uvec2 textureSize = textureSize(samplerWeed, 0);
    ...
    const uint grid_xy = accurate_xy.y * ACTUAL_GRID_WIDTH + accurate_xy.x;
    ...
    * vec2((u[indexOfVelocityU(accurate_xy)] + u[indexOfVelocityU(uvec2(accurate_xy.x-1, accurate_xy.y))])
    ...
    fColor = linearInterpolate2D(vec2(fract(before_advect_xy.x - uint(before_advect_xy.x)), fract(before_advect_xy.y - uint(before_advect_xy.y))), ll, lr, ul, ur);
