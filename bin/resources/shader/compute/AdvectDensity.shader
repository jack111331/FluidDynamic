...
void main() {
    ...
    // Linear interpolate
    density[grid_xy] = linearInterpolate2D(vec2(fract(before_advect_xy.x), fract(before_advect_xy.y)),
                                          prevQuantity[dot(lower_left_xy, uvec2(1, ACTUAL_GRID_WIDTH))],
                                          prevQuantity[dot(uvec2(upper_right_xy.x, lower_left_xy.y), uvec2(1, ACTUAL_GRID_WIDTH))],
                                          prevQuantity[dot(uvec2(lower_left_xy.x, upper_right_xy.y), uvec2(1, ACTUAL_GRID_WIDTH))],
                                          prevQuantity[dot(upper_right_xy, uvec2(1, ACTUAL_GRID_WIDTH))]);
