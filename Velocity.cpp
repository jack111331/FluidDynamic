...
void Velocity::clear() {
    for(int i = 0;i <= m_grid;++i) {
        for(int j = 0;j <= m_grid+1;++j) {
            m_uQuantity[m_currentContext][indexOfVelocityU(j, i, m_grid)] = m_vQuantity[m_currentContext][indexOfVelocityV(i, j, m_grid)] = 0.0f;
        }
    }
}
...
void Velocity::addForce(float dt) {
  for(int i = 1;i <= m_grid;++i) {
    for(int j = 1;j <= m_grid-1;++j) {
      m_uQuantity[m_currentContext][indexOfVelocityU(i, j, m_grid)] += dt * m_uQuantity[m_currentContext^1][indexOfVelocityU(i, j, m_grid)];
    }
  }
  ...
  for(int i = 1;i <= m_grid-1;++i) {
    for(int j = 1;j <= m_grid;++j) {
      m_vQuantity[m_currentContext][indexOfVelocityV(i, j, m_grid)] += dt * m_vQuantity[m_currentContext^1][indexOfVelocityV(i, j, m_grid)];
    }
  }
...
void Velocity::advectU(float dt, float *u, const float *prevU, const float *prevV) {
  ...
  for(int i = 1;i <= m_grid;++i) {
    for(int j = 1;j <= m_grid-1;++j) {
      ...
      float y = i - dt0 * 0.25f * (prevV[indexOfVelocityV(i-1, j, m_grid)] + prevV[indexOfVelocityV(i, j, m_grid)]
                                 + prevV[indexOfVelocityV(i-1, j+1, m_grid)] + prevV[indexOfVelocityV(i, j+1, m_grid)]);
...
void Velocity::advectV(float dt, float *v, const float *prevV, const float *prevU) {
  ...
  for(int i = 1;i <= m_grid-1;++i) {
    for(int j = 1;j <= m_grid;++j) {
      ...
      // use average staggered MAC grid velocity
      float x = j - dt0 * 0.25f * (prevU[indexOfVelocityU(i, j-1, m_grid)] + prevU[indexOfVelocityU(i, j, m_grid)]
                                 + prevU[indexOfVelocityU(i+1, j-1, m_grid)] + prevU[indexOfVelocityU(i+1, j, m_grid)]);
...
float Velocity::dVOverDX(int i, int j) {
    return (m_vQuantity[m_currentContext^1][indexOfVelocityV(i, j, m_grid)] - m_vQuantity[m_currentContext^1][indexOfVelocityV(i, j-1, m_grid)]);
}

float Velocity::dUOverDY(int i, int j) {
    return (m_uQuantity[m_currentContext^1][indexOfVelocityU(i, j, m_grid)] - m_uQuantity[m_currentContext^1][indexOfVelocityU(i-1, j, m_grid)]);
}
...
void Velocity::vorticityConfinement(float dt, float vorticity) {
    for(int i = 2;i < m_grid;++i) {
        for(int j = 2;j < m_grid;++j) {
            ...
            m_uQuantity[m_currentContext][indexOfVelocityU(i, j, m_grid)] = m_uQuantity[m_currentContext^1][indexOfVelocityU(i, j, m_grid)] + dt * force[0];
            m_vQuantity[m_currentContext][indexOfVelocityV(i, j, m_grid)] = m_vQuantity[m_currentContext^1][indexOfVelocityV(i, j, m_grid)] + dt * force[1];
        }
    }
   setUBoundary(m_uQuantity[m_currentContext], m_grid);
   setVBoundary(m_vQuantity[m_currentContext], m_grid);
...
void Velocity::massConserve(float dt, float materialDensity) {
  ...
  for(int i = 1;i <= m_grid;++i) {
    for(int j = 1;j <= m_grid;++j) {
      prevP[indexOfPressure(i, j, m_grid)] = -(m_uQuantity[m_currentContext][indexOfVelocityU(i, j, m_grid)] - m_uQuantity[m_currentContext][indexOfVelocityU(i, j-1, m_grid)] + m_vQuantity[m_currentContext][indexOfVelocityV(i, j, m_grid)] - m_vQuantity[m_currentContext][indexOfVelocityV(i-1, j, m_grid)]) / m_grid;
  ...
  for(int i = 1;i <= m_grid;++i) {
    for(int j = 1;j <= m_grid-1;++j) {
      m_uQuantity[m_currentContext][indexOfVelocityU(i, j, m_grid)] -= m_grid * (p[indexOfPressure(i, j+1, m_grid)] - p[indexOfPressure(i, j, m_grid)]);
    }
  }
  for(int i = 1;i <= m_grid-1;++i) {
    for(int j = 1;j <= m_grid;++j) {
      m_vQuantity[m_currentContext][indexOfVelocityV(i, j, m_grid)] -= m_grid * (p[indexOfPressure(i+1, j, m_grid)] - p[indexOfPressure(i, j, m_grid)]);
    }
  }
  delete prevP;
  delete p;
